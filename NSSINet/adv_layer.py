...
class DomainAdversarialLoss(nn.Module):
    ...
    def forward(self, x):
        ...
        d_s = d[0:2*num, :]
        d_t = d[2*num:, :]
        d_label_s = torch.ones(2*num, 1).to(x.device)
        d_label_t = torch.zeros(num, 1).to(x.device)
    ...
    #     d_s = d[0:num, :]
    #     d_t = d[num:, :]
    #     d_label_s = torch.ones(num, 1).to(x.device)
    #     d_label_t = torch.zeros(num, 1).to(x.device)
    #     self.domain_discriminator_accuracy = 0.5 * (binary_accuracy(d_s, d_label_s) + binary_accuracy(d_t, d_label_t))
    #     return 0.5 * (self.bce(d_s, d_label_s) + self.bce(d_t, d_label_t))
...
class GenderAdversarialLoss(nn.Module):

    def __init__(self,hidden_1, reduction: Optional[str] = 'mean',max_iter=1000):
        super(GenderAdversarialLoss, self).__init__()
        self.grl = WarmStartGradientReverseLayer(alpha=1.0, lo=0., hi=1., max_iters=max_iter, auto_step=True)
        self.domain_discriminator = Discriminator(hidden_1)
        self.bce = nn.BCELoss(reduction=reduction)
        self.domain_discriminator_accuracy = None

    def forward(self, x, label_sex):
        f = self.grl(x)
        d = self.domain_discriminator(f)
        source_num = int(len(x) / 2)
        d_s = d[0:source_num, :]
        d_t = d[source_num:, :]
        d_label_s = label_sex[0:source_num, :]
        d_label_t = label_sex[source_num:, :]
        self.domain_discriminator_accuracy = 0.5 * (binary_accuracy(d_s, d_label_s) + binary_accuracy(d_t, d_label_t))
        return 0.5 * (self.bce(d_s, d_label_s) + self.bce(d_t, d_label_t))